...
Three custom rules configured in Kibana's Security app:

- **SSH Brute Force** — fires on repeated failed SSH logins (`Medium`)
- **Sudo Execution** — fires whenever sudo is invoked (`Low`)
- **New User Created** — fires when a new account is added to the system (`High`)

## KQL queries documented

- Failed SSH authentication attempts
- Successful login after repeated failures
- All processes spawned by a specific user
